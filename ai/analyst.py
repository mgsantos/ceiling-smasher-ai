...
def parse_and_clean_inputs(red_team_text: str, *input_texts: str) -> list[str]:
    ...
    for line in red_team_text.split('\n'):
        # Look for: "• Ticker: ABORT" or "VERDICT: Ticker: ABORT" or "UEC: ABORT"
        if "ABORT" in line:
            # Clean up the line to find the ticker
            clean_line = line.replace('*', '').replace('•', '').replace('VERDICT:', '').strip()
            parts = clean_line.split(':')
            if len(parts) > 0:
                ticker_candidate = parts[0].strip().split(' ')[0] 
                if ticker_candidate.isalpha() and len(ticker_candidate) < 6:
                    banned_tickers.add(ticker_candidate)
    
    if banned_tickers:
        from utils.logger import logger
        print(f"[bold red]⛔ FIREWALL ACTIVE: Removing {banned_tickers}[/bold red]")
        logger.warning(f"Firewall removed: {banned_tickers}")
    ...
    def scrub_text(text):
        ...
        for line in text.split('\n'):
            ...
            for ban in banned_tickers:
                # If the banned ticker appears in the line, drop the whole line
...
def execute_portfolio_strategy(report_content: str, agent1_analysis: str, agent2_analysis: str, red_team_analysis: str, analysis_google: str, analysis_x: str = "", **kwargs) -> str:
    ...
    try:
        # Use c_report (Cleaned) instead of report_content
        prompt = _load_prompt("agent3_cio.txt", 
                              report_content=c_report, 
                              agent1_analysis=c_agent1,  # <--- Changed to Cleaned
                              agent2_analysis=c_agent2,  # <--- Changed to Cleaned
                              red_team_analysis=red_team_analysis, # Keep raw so CIO knows WHY they were banned
                              analysis_google=c_google,  # <--- Changed to Cleaned
                              analysis_x=c_x,            # <--- Changed to Cleaned
